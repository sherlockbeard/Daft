...
def write_deltalake(
    ...
    if delta_table is not None:
        if delta_table.protocol().min_writer_version > MAX_SUPPORTED_WRITER_VERSION:
            raise DeltaProtocolError(
                "This table's min_writer_version is "
                f"{delta_table.protocol().min_writer_version}, "
                "but this method only supports version 2."
            )

        invariants = delta_table.schema().invariants
        ...
        def check_data_is_aligned_with_partition_filtering(
            ...
            ] = delta_table._table.get_active_partitions()
            allowed_partitions: frozenset[frozenset[tuple[str, str | None]]] = delta_table._table.get_active_partitions(
                partition_filters
            )
            partition_values = pa.RecordBatch.from_arrays(
                [batch.column(column_name) for column_name in delta_table.metadata().partition_columns],
            ...
            for i in range(partition_values.num_rows):
                ...
                partition_map = {
                    ...
                    for column_name in delta_table.metadata().partition_columns
                }
